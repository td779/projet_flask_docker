...
const PLANNER =
  process.env.PLANNER !== undefined
    ? process.env.PLANNER
    : 'http://planner:3000'
const MULT = process.env.MULT !== undefined ? JSON.parse(process.env.MULT) : false
const ADD = process.env.ADD !== undefined ? JSON.parse(process.env.ADD) : false
const app = express()
...
app.listen(port, () => {
  register()
  console.log(`Worker ${id} listening at http://${host}:${port}`)
})
