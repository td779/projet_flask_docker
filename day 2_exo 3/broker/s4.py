...
def get_info(adresse = None, message = None):
  if message =="pong" :
    requests.post(f"http://server5-gateway:8574/{adresse}/{message}")        #version docker
  else :
...
if __name__ == "__main__":
    requests.post(server3_url + '/register', json={'address': f'http://server4-broker:{myport}', 'type': 'server4'})
