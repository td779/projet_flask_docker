...
def get_info(adresse = None, message = None):
  if message =="ping" :
    requests.post(f"http://localhost:1111/{adresse}/{message}")        #version docker
  #elif message =="pong" :
  else :
...
if __name__ == "__main__":
    requests.post(server3_url + '/register', json={'address': f'http://localhost:{myport}', 'type': 'server5'})
